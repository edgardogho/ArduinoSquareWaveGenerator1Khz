/**
   ...
   The frequency control is done using a serial protocol.
   

   This project is used to control 2 stepper motors and adjust their
   speed using a serial protocol. This project was created to aid a
   PLC controller that needs to interface with 2 stepper motors and 
   change their speed (2 presets speeds per channel).

   Values received can be stored on the ATmega EEPROM and restored 
   on power on.
   ...
   Protocol:
   Fwwwwxxxxyyyyzzzz\n : Set Frequency (each digit is Ascii for 0~9)
                        wwww = Channel 1 Fast 
                        xxxx = Channel 1 Slow
                        yyyy = Channel 2 Fast
                        zzzz = Channel 2 Slow
                        
  S\n                  : Status? (returns frequencies)

  W\n                  : Save values to EEPROM

  Serial Pins are Arduino regular Tx/Rx at 9600 bauds 8,N,1.
...
unsigned int channel1Counter = 976;
unsigned int channel1Freq = 1023;
unsigned int channel1Fast = 1023;
unsigned int channel1Slow = 512;
const uint8_t OC1A = 9;
const uint8_t fast1Select = 2;

unsigned int channel2Counter = 976;
unsigned int channel2Freq = 1023;
unsigned int channel2Fast = 1023;
unsigned int channel2Slow = 512;
...
void setup() {
  ...
  EEPROM.get(0, channel1Fast);
  EEPROM.get(2, channel1Slow);
  EEPROM.get(4, channel2Fast);
  EEPROM.get(6, channel2Slow);
...
// Handler for Serial Port event.
//   This is triggered each time there is an event with
//   the serial port.
void serialEvent() {
  ...
  while (Serial.available()) {
    ...
    // Command ends with \n , so if we detect a \n raise a flag
    // so outside from this interrupt we can deal with the new command.
    if (inChar == '\n') {
      // Raise the flag
      bufferReady = true;
    }

    // Make a safety check so we don't fill the buffer.
    if (bufferCounter>23){
      //Something is not right, we got 20 bytes but no \n
      // so we might have received a broken message
      bufferCounter=0;
      inputBuffer="";
      bufferReady=false;
    }
...
void loop() {
  ...
  //Check for serial flag indicating there is a valid command
  if (bufferReady == true){
    bufferReady = false;
    ...
    if (inputBuffer.charAt(0)=='F' && inputBuffer.length() >= 18){
      ...
      channel2Slow = inputBuffer.substring(14,17).toInt();

      Serial.print("Setting: ");
      Serial.print(channel1Fast);
      Serial.print(",");
      Serial.print(channel1Slow);
      Serial.print(",");
      Serial.print(channel2Fast);
      Serial.print(",");
      Serial.print(channel2Slow);
      Serial.print("\n");
      
    } else if (inputBuffer.charAt(0) == 'W'){
      Serial.print("Writing EEPROM\n");
      EEPROM.put(0, channel1Fast);
      EEPROM.put(2, channel1Slow);
      EEPROM.put(4, channel2Fast);
      EEPROM.put(6, channel2Slow);
